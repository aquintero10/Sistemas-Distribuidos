    ...
    class btcMiner
    {
        ...
        class Pool
        {
            ...
            private byte[] ParseData(string json)
            {
                ...
                if (match.Success)
                {
                    string data = btcMiner.RemovePadding(match.Groups[1].Value);
                    data = btcMiner.EndianFlip32BitChunks(data);
                    return btcMiner.ToBytes(data);
                }
            ...
            {
                string data = btcMiner.EndianFlip32BitChunks(btcMiner.ToString(share));
                string paddedData = btcMiner.AddPadding(data);
        ...
        class Program
        {
            ...
            private static void SendShare(byte[] share)
            {
                ...
                Print("Share: " + btcMiner.ToString(_work.Current));
                Print("Nonce: " + btcMiner.ToString(_nonce));
                Print("Hash: " + btcMiner.ToString(_work.Hash));
            ...
            {
                ClearConsole();
                Print("Data: " + btcMiner.ToString(_work.Data));
                string current = btcMiner.ToString(_nonce);
                string max = btcMiner.ToString(uint.MaxValue);
